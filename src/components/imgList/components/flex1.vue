<template>
  <div>
    <ul class="list-wrapper" :class="`list-${datas.moduleStyle.columnNum}`">
      <li v-for="item in curImgList" :key="item.id" @click="matchjumpUrl(item)">
        <div class="img-wrapper">
          <img :src="item.imgUrl" alt="" />
        </div>
        <div class="text">
          <p class="name" v-if="datas.moduleStyle.showImgName && item.name !== ''">
            {{ item.name }}
          </p>
          <p class="detail" v-if="datas.moduleStyle.showImgDetail && item.detail !== ''">
            {{ item.detail }}
          </p>
        </div>
      </li>
