<template>
  <div class="wrapper">
    <div class="top-image" @click="jumpDetail(topImg.value)" v-if="datas.headerstyle === 2">
      <img :src="topImg.value.image" alt="" />
      <div class="essay-title">{{ topImg.value.title }}</div>
    </div>

    <div class="list" :class="`list-${datas.arrangeUse}`" v-if="list.value.length">
      <div class="item" @click="jumpDetail(item)" v-for="item in list.value" :key="item.id">
...
export default {
  ...
  setup(props) {
    ...
    const jumpDetail = (item) => {
      router.push(`/essayDetail?essayId=${item.id}`);
    };
